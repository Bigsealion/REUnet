...
# edit from save_segmentation_nifti_from_softmax, but save feature map rather than segmentation
# do not argmax
def save_feature_map_nifti(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    if bbox is not None:
        if len(seg_old_spacing.shape) == 3:
        ...
        elif len(seg_old_spacing.shape) == 4:
            seg_old_spacing = seg_old_spacing.transpose((1, 2, 3, 0))
            seg_old_size = np.zeros(list(shape_original_before_cropping) + [seg_old_spacing.shape[3]], dtype=np.float32)
            for c in range(3):
                # seg_old_spacing is 4d in there
                bbox[c][1] = np.min((bbox[c][0] + seg_old_spacing.shape[c], shape_original_before_cropping[c]))
            seg_old_size[
                bbox[0][0]:bbox[0][1],
                bbox[1][0]:bbox[1][1],
            ...
            # if length dim 0 is 1, remove it
            if seg_old_spacing.shape[3] == 1:
                seg_old_size = seg_old_size[:, :, :, 0]
        else:
            raise RuntimeError("Unsupport feature map dim to save!")
    else:
        seg_old_size = seg_old_spacing
        
    # may not be used in my process
    ...
    # save 4d image
    if len(seg_old_size_postprocessed.shape) == 3:
    ...
    elif len(seg_old_size_postprocessed.shape) == 4:
        seg_resized_itk = sitk.GetImageFromArray(seg_old_size_postprocessed.astype(np.float32))
        seg_resized_itk.SetSpacing(list(properties_dict['itk_spacing']) + [1])
        seg_resized_itk.SetOrigin(list(properties_dict['itk_origin']) + [0])
        seg_resized_itk.SetDirection(properties_dict['itk_direction'])
        sitk.WriteImage(seg_resized_itk, out_fname)
        ...
        # should not be used
        raise RuntimeError("Unsupport feature map dim!")